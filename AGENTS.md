...
- **The assistant transcript is stored per project, outside the project document**: `src/store/assistant.ts` writes it to IndexedDB under `pixeldeck-chat:<projectId>`. It must never enter the project JSON (it is not design data and must not travel in an export), and what is stored deliberately drops `projectAfter` (a whole second copy of the project) and any captured `image`. A restored turn always comes back with `undoable: false`, because the undo stack it referred to did not survive the reload.
- **`partialSay` is not a streaming JSON parser, on purpose**: it reads one known string field out of a half-arrived buffer so the sentence can be shown while it is written. The authoritative read is still `parseAgentReply` on the finished text. Do not grow it into a general parser.
- **`chatStream` falls back rather than failing**: streaming is attempted only for the OpenAI-compatible SSE shape; Google's native endpoint and any gateway that rejects `stream` fall back to the ordinary request with one delta carrying the whole answer. A transport that does not implement `chatStream` at all (an injected one, or a test's) falls back the same way through `transportChatStream`. Do not make streaming a requirement.
