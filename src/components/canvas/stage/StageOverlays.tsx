...
export function StageChrome({
  ...
  return <>
    ...
    {editingGroupId && <div style={{ position: 'absolute', top: 12, left: '50%', transform: 'translateX(-50%)', display: 'flex', alignItems: 'center', gap: 10, background: 'rgba(124,110,246,0.95)', borderRadius: 8, padding: '6px 14px', zIndex: 100, pointerEvents: 'all', boxShadow: '0 4px 20px rgba(124,110,246,0.4)' }}>
      <span style={{ color: '#fff', fontSize: 12, fontWeight: 600, display: 'flex', alignItems: 'center', gap: 6 }}><Icon name="group" size={13} />{t('canvas.editingGroup')}</span>
      <button onClick={(e) => { e.stopPropagation(); exitGroupEdit() }} style={{ background: 'rgba(255,255,255,0.2)', border: 'none', borderRadius: 4, color: '#fff', cursor: 'pointer', fontSize: 11, fontWeight: 600, padding: '3px 10px' }}>{t('canvas.exitGroup')}</button>
    </div>}
    {/* Anchored from CSS (.pd-canvas-zoom) so it can move out of the way of the
        selection toolbar on narrow screens and follow the writing direction. */}
    <div className="pd-canvas-zoom" style={{ display: 'flex', alignItems: 'center', gap: 4, background: 'rgba(15,15,19,0.88)', backdropFilter: 'blur(8px)', borderRadius: 8, padding: '4px 8px', border: '1px solid rgba(255,255,255,0.1)', zIndex: 100, pointerEvents: 'all' }}>
      <button onClick={(e) => { e.stopPropagation(); setZoom(Math.max(0.05, zoom - 0.02)) }} title={t('canvas.zoomOut')} style={{ background: 'none', border: 'none', color: '#e8e8f0', cursor: 'pointer', padding: '2px 4px', lineHeight: 1, display: 'flex', alignItems: 'center' }}><Icon name="minus" size={14} strokeWidth={2.2} /></button>
      <input type="text" value={zoomInput ?? `${Math.round(zoom * 100)}`} onChange={(e) => setZoomInput(e.target.value)} onFocus={(e) => { setZoomInput(`${Math.round(zoom * 100)}`); e.target.select() }} onBlur={() => { const parsed = parseInt(zoomInput ?? '', 10); if (!isNaN(parsed) && parsed > 0) setZoom(Math.max(0.05, Math.min(4, parsed / 100))); setZoomInput(null) }} onKeyDown={(e) => { if (e.key === 'Enter') (e.target as HTMLInputElement).blur(); if (e.key === 'Escape') { setZoomInput(null); (e.target as HTMLInputElement).blur() }; e.stopPropagation() }} onClick={(e) => e.stopPropagation()} title={t('canvas.zoomLevel')} style={{ background: 'none', border: 'none', color: '#6b6b7a', fontSize: 11, width: 36, textAlign: 'center', cursor: 'text', outline: 'none', padding: 0 }} />
      <span style={{ color: '#6b6b7a', fontSize: 11, marginInlineStart: -2 }}>%</span>
      <button onClick={(e) => { e.stopPropagation(); setZoom(Math.min(4, zoom + 0.02)) }} title={t('canvas.zoomIn')} style={{ background: 'none', border: 'none', color: '#e8e8f0', cursor: 'pointer', padding: '2px 4px', lineHeight: 1, display: 'flex', alignItems: 'center' }}><Icon name="plus" size={14} strokeWidth={2.2} /></button>
      <div style={{ width: 1, height: 14, background: 'rgba(255,255,255,0.12)', margin: '0 2px' }} />
      <button onClick={(e) => { e.stopPropagation(); handleFit() }} title={t('canvas.fitTitle')} style={{ background: 'none', border: 'none', color: '#a0a0b0', cursor: 'pointer', fontSize: 11, padding: '0 4px', lineHeight: 1 }}>{t('canvas.fit')}</button>
    </div>
    ...
    {spaceDown && <div style={{ position: 'absolute', bottom: 12, left: '50%', transform: 'translateX(-50%)', background: 'rgba(15,15,19,0.88)', backdropFilter: 'blur(8px)', borderRadius: 8, padding: '4px 12px', border: '1px solid rgba(255,255,255,0.1)', color: '#6b6b7a', fontSize: 11, zIndex: 100, pointerEvents: 'none', display: 'flex', alignItems: 'center', gap: 6 }}><Icon name="hand" size={12} />{t('canvas.panMode')}</div>}
