...
import { isNativeApp, isSaveCancelled, saveFile } from '@/utils/saveFile'
...
export function ExportModal({ open, onClose, stageRef }: ExportModalProps) {
  ...
  const folderSupported = !isNativeApp() && Boolean(window.showDirectoryPicker)
  ...
  const downloadResults = async (results: ProjectImageExportResult[]) => {
    for (const item of results) {
      if (abortControllerRef.current?.signal.aborted) throw new ExportCancelledError()
      await downloadDataUrl(item.dataUrl, item.relativePath.replace(/\//g, '__'))
  ...
  const downloadZip = async (results: ProjectImageExportResult[]) => {
    ...
    await saveFile(zipBlob, zipFileNameFor(project.name))
  }
  ...
  const handleRunExport = async () => {
    ...
    try {
      ...
      // Browser pickers require transient user activation: ask before rendering.
      const dirHandle = exportOutput === 'folder' && window.showDirectoryPicker
      ...
      } else if (exportOutput === 'folder' && dirHandle) {
        try {
          let written = 0
          for (const item of results) {
      ...
      if (err instanceof ExportCancelledError || isSaveCancelled(err)) {
        setExportError(null)
      } else {
