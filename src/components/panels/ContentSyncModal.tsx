...
export function ContentSyncModal({ open, onClose }: ContentSyncModalProps) {
  ...
  return <ModalShell open={open} onClose={onClose} title="Bring content from" maxWidth="max-w-md" bodyClassName="px-5 py-5" footerClassName="flex justify-end gap-2 border-t border-[rgba(255,255,255,0.06)] px-5 py-4" footer={<><button type="button" onClick={onClose} className="rounded-lg px-3 py-2 text-xs text-[#8f90a3] hover:bg-[rgba(255,255,255,0.06)] hover:text-white">Cancel</button><button type="button" onClick={handleConfirm} disabled={!plan || plan.updatedCount === 0} className="rounded-lg bg-[var(--pd-c-7c6ef6)] px-3 py-2 text-xs font-medium text-white hover:bg-[#6c5ed6] disabled:cursor-not-allowed disabled:opacity-50">Bring content</button></>}>
    ...
    {candidates.length > 1 && <label className="block text-[10px] font-semibold uppercase tracking-[0.15em] text-[var(--pd-c-6b6b7a)]">Bring content from<select value={selectedFamily} onChange={(event) => setSourceFamily(event.target.value as typeof candidates[number])} className="mt-2 w-full rounded border border-[rgba(255,255,255,0.1)] bg-[var(--pd-c-0f0f13)] px-2 py-1.5 text-xs font-normal normal-case tracking-normal text-[var(--pd-c-e8e8f0)] focus:outline-none">{candidates.map((family) => <option key={family} value={family}>{FORMAT_FAMILY_LABELS[family]}</option>)}</select></label>}
    ...
    {updates.length > 0 && <div className="mt-4 max-h-52 overflow-y-auto rounded-lg border border-[rgba(255,255,255,0.08)] bg-[var(--pd-c-0f0f13)]">{updates.map((entry) => <div key={entry.path.join('.')} className="flex items-center gap-2 border-b border-[rgba(255,255,255,0.06)] px-3 py-2 text-xs last:border-b-0"><span className="min-w-0 flex-1 truncate text-[var(--pd-c-e8e8f0)]">{entry.layerName}</span><span className="text-[var(--pd-c-6b6b7a)]"><Icon name="arrow-left" size={12} /></span><span className="min-w-0 flex-1 truncate text-end text-[#a6a7b8]">{entry.preview ?? entry.changedKeys.join(', ')}</span></div>)}</div>}
