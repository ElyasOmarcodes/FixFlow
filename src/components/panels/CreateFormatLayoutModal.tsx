...
export function CreateFormatLayoutModal({ open, targetFormatId, onClose }: CreateFormatLayoutModalProps) {
  ...
  return <ModalShell open={open} onClose={onClose} title={`Start your ${FORMAT_FAMILY_LABELS[targetFamily]} layout`} maxWidth="max-w-md" bodyClassName="px-5 py-5" footerClassName="flex justify-end gap-2 border-t border-[rgba(255,255,255,0.06)] px-5 py-4" footer={<><button type="button" onClick={onClose} className="rounded-lg px-3 py-2 text-xs text-[#8f90a3] hover:bg-[rgba(255,255,255,0.06)] hover:text-white">Cancel</button><button type="button" onClick={handleCreate} disabled={sourceGroups.length === 0} className="rounded-lg bg-[var(--pd-c-7c6ef6)] px-3 py-2 text-xs font-medium text-white hover:bg-[#6c5ed6] disabled:cursor-not-allowed disabled:opacity-50">Create {FORMAT_FAMILY_LABELS[targetFamily]} layout</button></>}>
    ...
    {sourceCandidates.length > 0 && <label className="mt-4 block text-[10px] font-semibold uppercase tracking-[0.15em] text-[var(--pd-c-6b6b7a)]">Copy from<select value={sourceFormat} onChange={(event) => setSourceFormat(event.target.value as CanvasFormatId)} className="mt-2 w-full rounded border border-[rgba(255,255,255,0.1)] bg-[var(--pd-c-0f0f13)] px-2 py-1.5 text-xs font-normal normal-case tracking-normal text-[var(--pd-c-e8e8f0)] focus:outline-none">{sourceCandidates.map((format) => { const count = selectForkSourceGroups(store.project, format, targetFormatId).length; const label = format === BASE_CANVAS_FORMAT ? 'Unscoped screens' : getFormatLabel(format, store.project.settings.customFormats); return <option key={format} value={format}>{label} ({count} screen{count === 1 ? '' : 's'})</option> })}</select></label>}
