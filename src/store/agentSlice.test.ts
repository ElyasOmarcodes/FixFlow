...
describe('an assistant turn is one undo step', () => {
  it('records a single entry however many layers the turn touched', () => {
    const store = useEditorStore.getState()
    store.beginAgentTurn()
    runTool('add_layer', { layerType: 'text', text: 'One' })
    runTool('add_layer', { layerType: 'text', text: 'Two' })
    runTool('add_layer', { layerType: 'icon', icon: 'star' })
    runTool('set_background', { fill: '#101020' })
  ...
  it('puts the canvas back exactly as it was on one undo', () => {
    useEditorStore.getState().addText()
    const before = layers().length
    useEditorStore.temporal.getState().clear()

    useEditorStore.getState().beginAgentTurn()
    runTool('add_layer', { layerType: 'text', text: 'From the assistant' })
    runTool('add_layer', { layerType: 'chip', text: 'New' })
  ...
  it('records nothing when the turn only read', () => {
    useEditorStore.getState().beginAgentTurn()
    runTool('read_slide', {})
    runTool('read_project', {})
    expect(useEditorStore.getState().endAgentTurn()).toBe(false)
    expect(historyDepth()).toBe(0)
  })

  it('rolls a failed turn back without leaving half a change behind', () => {
    const before = layers().length
    useEditorStore.getState().beginAgentTurn()
    runTool('add_layer', { layerType: 'text', text: 'Half done' })
    runTool('set_background', { fill: '#FF0000' })
  ...
  it('leaves history recording again after the turn, so ordinary edits still undo', () => {
    useEditorStore.getState().beginAgentTurn()
    runTool('add_layer', { layerType: 'text', text: 'Turn' })
    useEditorStore.getState().endAgentTurn()
    useEditorStore.getState().addText()
    expect(historyDepth()).toBe(2)
  })

  it('resumes history after a rollback too', () => {
    useEditorStore.getState().beginAgentTurn()
    runTool('add_layer', { layerType: 'text', text: 'Turn' })
    useEditorStore.getState().rollbackAgentTurn()
    useEditorStore.getState().addText()
  ...
  it('does not nest: a second begin while one is open is ignored', () => {
    const store = useEditorStore.getState()
    store.beginAgentTurn()
    runTool('add_layer', { layerType: 'text', text: 'First' })
    store.beginAgentTurn()
    runTool('add_layer', { layerType: 'text', text: 'Second' })
    useEditorStore.getState().endAgentTurn()
  ...
  it('reports the turn as open while it runs, so the UI can show it', () => {
